...
Use Windows GitHub Actions for reproducible build/test/package and isolated hidden rendering validation. For this project, those pipeline checks are sufficient for release completion unless the user explicitly requests a live Vanilla/Gepard/RDP check. Never label hosted runner evidence as live gameplay. Do not introduce paid runners or unnecessary external testing services. Before publishing a UI release, run `scripts/test-ui-layout.ps1` locally or on a Windows runner in background-safe mode, fix failures and inspect the generated mock screenshots. Never show test windows, observe live clients or send desktop input during background-only development. A successful build alone is not UI validation.
...
- Exception: two fresh exact `Server Closed.(1)` Message observations enter server-outage mode. Pending accounts share one ordinary verified launcher/login retry every 15 minutes until the recovering client's expected gameplay identity is freshly verified. Do not infer availability from `Please wait...`, missing frames or a vanished dialog. Waiting releases input/recovery ownership and leaves healthy siblings alone; STOP and ownership/configuration changes cancel stale work. Show the next-check status in the existing recovery UI/log.
