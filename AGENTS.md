...
when reconnect supervision, Cart or Mail is OFF. When one fresh, coherent,
verified read-only snapshot shows carried weight strictly above 50%, SP strictly
below 25% and HP strictly below 50% together, immediately close only that exact
client using the creation-time-pinned Windows process handle. Do not wait for
Cart retry, graceful-window-close delays, or another client's input lease.
Never bypass a denied native operation or infer successful exit from a failure.
This rule overrides Cart damage-abort resume/retry and all recovery/resume paths.
Cancel affected input and retain its lease until held mouse/keys unwind; leave
healthy siblings alone. Log raw HP/SP/weight ratios, identity, trigger and close
result. Persist the emergency hold across STOP/START, settings edits, updates
and application restart. No automatic relaunch or farming resume until the user
explicitly clears the emergency hold; ordinary Weight/Cart hold-clear must not
clear it. Unknown, stale, incoherent or mismatched state authorizes no close.
